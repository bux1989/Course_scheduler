    ...
    <div class="scheduler-grid" role="grid" aria-label="School schedule grid" :data-readonly="isReadOnly">
        ...
        <div class="main-grid-container">
            ...
            <div class="grid-body">
                ...
                >
                    ...
                    >
                        ...
                        <div v-if="safeLength(getCellAssignments(day.id, period.id)) > 0" class="assignments-container">
                            <!-- Context menu disabled in favor of inline buttons -->
                            <div
                            ...
                            >
                                <div v-if="!isEditing(assignment.id)" class="assignment-content">
                                    <div class="assignment-info">
                                        ...
                                        <span class="meta-line">
                                            ...
                                            <span class="room-name" v-if="assignment.room_id"
                                                >• {{ getRoomName(assignment.room_id) }}</span
                                            >
                                        </span>
                                    </div>
                                    <!-- Edit/Delete buttons -->
                                    <div class="assignment-actions" v-if="!isReadOnly && !isLiveMode">
                                        <button
                                        ...
                                        <button
                                            ...
                                            @click.stop="deleteInlineAssignment(assignment)"
                                    ...
                                    <!-- View button for live/read-only mode -->
                                    <div class="assignment-actions" v-else>
                                        <button
                                            class="action-btn view-btn"
                                            @click.stop="startInlineEditReadOnly(assignment, day.id, period.id)"
                ...
                <div v-if="showStatistics && focusedPeriodId" class="statistics-section">
                    <!-- Weekly totals title spanning all columns -->
                    ...
                    <div class="statistics-table">
                        ...
                        <div class="icon-headers-row">
                            <!-- Weekly totals block -->
                            <div class="header-block weekly-block">
                                <div class="header-cell"></div>
                                <div class="header-cell" title="Total free spots available">📊</div>
                                <div class="header-cell" title="Average spots available">⚖️</div>
                                <div class="header-cell" title="Courses available">📚</div>
                            </div>
                            <!-- Weekday blocks -->
                            <div v-for="day in visibleDays" :key="`header-${day.id}`" class="header-block day-block">
                                <div class="header-cell"></div>
                                <div class="header-cell" title="Total free spots available">📊</div>
                                <div class="header-cell" title="Average spots available">⚖️</div>
                                <div class="header-cell" title="Courses available">📚</div>
                        ...
                        <!-- Grade rows (1-6) -->
                        <div v-for="grade in [1, 2, 3, 4, 5, 6]" :key="`grade-${grade}`" class="grade-row">
                        ...
                        <div class="sum-row">
                            ...
                            <div v-for="day in visibleDays" :key="`sum-${day.id}`" class="stats-block day-block">
                                <div class="sum-label">∑:</div>
                                <div class="sum-value">{{ formatInt(getDailyTotals(day.id).totalSpots) }}</div>
                                <div class="sum-value">{{ formatInt(getDailyTotals(day.id).averageSpots) }}</div>
                                <div class="sum-value">{{ formatInt(getDailyTotals(day.id).totalCourses) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div v-if="safeLength(getNoPreferredDaysCourses()) > 0" class="no-preferred-days-panel">
                ...
                <div class="no-preferred-courses-list">
                    <div
                        ...
                        :style="getCourseCardStyle(course)"
                        @click="
                            focusedPeriodId ? assignCourseToSlot(course, visibleDays[0]?.id, focusedPeriodId) : null
                        "
                    >
                        <div class="course-name">{{ course.name || course.course_name || course.title }}</div>
                        <div class="course-details">
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const getCourseName = id =>
            id
                ? find(props.courses, id)?.name ||
                  find(props.courses, id)?.course_name ||
                  find(props.courses, id)?.title ||
                  null
                : null;
        const getSubjectName = id =>
            id
                ? find(props.subjects, id)?.name ||
                  find(props.subjects, id)?.title ||
                  find(props.subjects, id)?.subject_name ||
                  null
                : null;
        ...
        const getTeacherNames = ids =>
            !ids?.length
                ? ''
                : ids
                      .map(id => find(props.teachers, id)?.name)
                      .filter(Boolean)
                      .join(', ');
        ...
        const getCellAssignments = (dayId, periodId) => {
            const list = currentSchedules.value.filter(
        ...
        const normalizeSlots = course => {
            ...
            const pushIfValid = (d, p) => {
                if (d != null && p != null) out.push({ dayId: String(d), periodId: String(p) });
            };
        ...
        const buildAllowedSlotsLabel = course => {
            ...
            return Object.entries(byDay)
                .map(([dn, per]) => `${dn} (${Array.from(new Set(per)).join(', ')})`)
                .join('; ');
        };
        ...
        const deleteAssignmentFromContext = () => {
            ...
            if (!a || props.isReadOnly || props.isLiveMode) {
                closeContextMenu();
                return;
            }
        ...
        const handleAssignmentDragStart = (event, a, dayId, periodId) => {
            ...
            event.dataTransfer.setData(
                'text/plain',
                JSON.stringify({ type: 'assignment', assignment: a, originalDayId: dayId, originalPeriodId: periodId })
            );
        ...
        const getProposedTeacherIds = course => {
            ...
            if (
                course?.possible_staff_ids &&
                typeof course.possible_staff_ids === 'object' &&
                !Array.isArray(course.possible_staff_ids)
            ) {
        ...
        const getProposedRoomId = course => {
            ...
            const firstValidId = idCandidates.find(
                x => x != null && x !== 'null' && x !== 'undefined' && String(x).trim() !== ''
            );
            if (firstValidId) {
        ...
        const getNoPreferredDaysCourses = () =>
            safeArray(props.courses).filter(course => normalizeSlots(course).length === 0);
        ...
        // Calculate weekly totals for grades 1-6
        const getWeeklyGradeTotals = () => {
            ...
            grades.forEach(grade => {
                ...
                // Sum across all days and all periods
                visibleDays.value.forEach(day => {
                    visiblePeriods.value.forEach(period => {
                        if (period.is_instructional) {
                            const scheduledCourses = getScheduledCoursesForSlot(day.id, period.id);
                            scheduledCourses.forEach(course => {
                                const grades = parseGrades(course);
                                if (grades.includes(grade)) {
                                    totalCourses++;
        ...
        const getDailyTotals = dayId => {
            ...
            // Sum across all instructional periods for this day
            visiblePeriods.value.forEach(period => {
                if (period.is_instructional) {
                    const scheduledCourses = getScheduledCoursesForSlot(dayId, period.id);
                    totalCourses += scheduledCourses.length;
                    scheduledCourses.forEach(course => {
                        const spots = course.freeSpots || 0;
                        totalSpots += spots;
                        totalGradeAllocation += spots;
                    });
                }
            });
        ...
        // Calculate weekly period sums (totals across all grades for the selected period)
        const getWeeklyPeriodSums = periodId => {
            const weeklyTotals = getWeeklyPeriodTotals(periodId);
            let totalSpots = 0;
            let totalCourses = 0;
            let averageSpots = 0;

            weeklyTotals.forEach(gradeTotal => {
                totalSpots += gradeTotal.totalSpots;
                totalCourses += gradeTotal.coursesCount;
                averageSpots += gradeTotal.averageSpots;
            });
...
.day-statistics-cell:last-child {
    border-right: 0;
}
.stats-headers {
    display: grid;
...
.day-courses-column:first-of-type {
    /* ensure line between period col and first day */
    border-left: 1px solid #ddd;
}
.available-courses-list {
    display: flex;
