...
export default {
  ...
  props: {
    ...
    // Proposed teachers to preselect (possible_staff_ids from the course)
    preselectedTeacherIds: { type: Array, default: () => [] },
    // NEW: Proposed room to preselect (possible_room_id from the course)
  ...
  setup(props, { emit }) {
    ...
    // Initialize selection from preselectedTeacherIds AND preselectedRoomId
    function initPreselection() {
      ...
      if (props.preselectedRoomId) {
        const roomId = asId(props.preselectedRoomId);
        const roomExists = props.rooms.some(r => asId(r.id) === roomId);
        if (roomExists) {
